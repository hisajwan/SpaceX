...
function Container({launches, landing, loading}) {
    return (
        <section className={blockName}>
            {loading ? <h1>Loading...</h1> : (launches && launches.length > 0 ? launches.map(launch => {
                const land = landing ? landing : launch.rocket.first_stage.cores.length > 1 && !!launch.rocket.first_stage.cores.find(core => core.land_success === true);
                ...
                return (
                    <Card key={launch.flight_number} customClass= {`${blockName}__card`}>
                        ...
                        <div className={`${blockName}__card-body`} aria-labelledby="Mission Details" >
                            ...
                            <div>Successful Landing: <span aria-labelledby={`launch  success ${land.toString()}`}>{land.toString()}</span></div>
                        </div>
                    </Card>
                )
            }) : <h4> No data found</h4>)}
